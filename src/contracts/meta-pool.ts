...
import {ContractState, GetAccountInfoResult, LiquidUnstakeResult, RemoveLiquidityResult, VLoanInfo} from "./meta-pool-structs"
...
export class MetaPool extends SmartContract {
    ...
    get_account_info(accountId?:string) : Promise<GetAccountInfoResult> {
        return this.view("get_account_info",{ account_id: accountId||this.wallet.getAccountId() } ) 
    }
    ...
    liquid_unstake(stnearToBurn:number, minExpectedNear:number) : Promise<LiquidUnstakeResult> {
        return this.call("liquid_unstake", {"stnear_to_burn":ntoy(stnearToBurn), "min_expected_near":ntoy(minExpectedNear)}, TGas(75) ) //1 yocto hack
    }
